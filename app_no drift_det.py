...
# ---------------------------------------------------------------------------
def create_performance_time_series(batch_df, summary):
...
def create_fairness_metrics_chart(fairness_df, metric_type='demographic_parity'):
    ...
    fig.update_layout(
        ...
        font=dict(color=TEXT_MUTED, size=11),
        xaxis=dict(showgrid=True, gridcolor="#222938", zeroline=False),
        yaxis=dict(showgrid=True, gridcolor="#222938", zeroline=False, title=y_title)
    )
    
    return fig

def create_approval_rate_breakdown(fairness_history):
...
def create_alert_timeline(alerts_df, severity_filter='all'):
    """Create alert timeline visualization"""
    if severity_filter != 'all':
        filtered_df = alerts_df[alerts_df['severity'] == severity_filter]
    else:
...
def create_drift_status_donut(drift_reports):
...
def create_graph_card(title, graph_id, initial_figure=None):
    return html.Div(
        style={
            "backgroundColor": CARD_BG, "borderRadius": "16px", "padding": "24px",
        ...
        children=[
            ...
            dcc.Graph(id=graph_id, figure=initial_figure if initial_figure else go.Figure(), 
...
app.layout = html.Div(
    ...
    children=[
        # Store for data
        dcc.Store(id='monitoring-data-store'),
        
        # Interval component for auto-refresh (30 seconds)
        dcc.Interval(
        ...
        dbc.Container(
            ...
            children=[
                ...
                dbc.Row(
                    ...
                    children=[
                        dbc.Col(
                            html.Div(
                                style={
                                    "backgroundColor": CARD_BG, "borderRadius": "16px", "padding": "16px 24px",
                                ...
                                children=[
                                    ...
                                    dcc.Dropdown(
                                        ...
                                        options=[
                                            ...
                                            {'label': 'Disparate Impact', 'value': 'disparate_impact'},
                                            {'label': 'Equal Opportunity', 'value': 'equal_opportunity'}
                                        ],
                                        value='demographic_parity',
                                        style={"width": "250px", "display": "inline-block"},
                                        clearable=False
                                    ),
                                    html.Span("ALERT FILTER: ", style={"color": TEXT_MUTED, "fontSize": "13px", "fontWeight": "600", "marginLeft": "30px", "marginRight": "10px"}),
                                    dcc.Dropdown(
                ...
                dbc.Row(
                    ...
                    children=[
                        dbc.Col(create_graph_card(
                            "Fairness Metric Over Time (Select Above)", 
...
def update_navbar_status(data):
    """Update navbar status"""
    if not data:
        return html.Span("LOADING...", style={"color": ACCENT_ORANGE, "fontSize": "12px", "fontWeight": "700"})
    
    summary = data['summary']
    return html.Span(
        f"MONITORING: {summary['monitoring_summary']['total_batches_processed']} BATCHES", 
        style={"color": ACCENT_GREEN, "fontSize": "12px", "fontWeight": "700", 
               "letterSpacing": "1px", "backgroundColor": "rgba(0, 230, 118, 0.1)", 
               "padding": "6px 14px", "borderRadius": "20px"}
    )
...
def update_performance_chart(data):
    """Update performance time series"""
    if not data:
        return go.Figure()
    
    batch_df = prepare_batch_dataframe(data['batch_history'])
    return create_performance_time_series(batch_df, data['summary'])

@app.callback(
    Output('fairness-metrics-chart', 'figure'),
    [Input('monitoring-data-store', 'data'),
     Input('fairness-metric-dropdown', 'value')]
)
def update_fairness_chart(data, metric_type):
    """Update fairness metrics chart based on selection"""
    if not data:
        return go.Figure()
    
    fairness_df = prepare_fairness_dataframe(data['fairness_history'])
    return create_fairness_metrics_chart(fairness_df, metric_type)

@app.callback(
    Output('approval-breakdown-chart', 'figure'),
    Input('monitoring-data-store', 'data')
)
def update_approval_breakdown(data):
    """Update approval breakdown chart"""
    if not data:
        return go.Figure()
    
    return create_approval_rate_breakdown(data['fairness_history'])

@app.callback(
    Output('alert-timeline-chart', 'figure'),
    [Input('monitoring-data-store', 'data'),
     Input('alert-severity-dropdown', 'value')]
)
def update_alert_timeline(data, severity_filter):
    """Update alert timeline with filter"""
    if not data:
        return go.Figure()
    
    alerts_df = prepare_alerts_dataframe(data['alerts'])
    return create_alert_timeline(alerts_df, severity_filter)

@app.callback(
    Output('drift-status-donut', 'figure'),
    Input('monitoring-data-store', 'data')
)
def update_drift_donut(data):
    """Update drift status donut"""
    if not data:
        return go.Figure()
    
    return create_drift_status_donut(data['drift_reports'])
...
def update_alert_table(data, severity_filter):
    """Update alert table"""
    if not data:
